...
class VideoDatabaseRepository(
    ...
    override suspend fun create(video: Video) {
        dbClient.sql(VideoSqlStatements.INSERT)
            .bind("id", video.id)
    ...
    override suspend fun findById(id: String): Video? =
        dbClient.sql(VideoSqlStatements.FIND_BY_ID)
            .bind("id", id)
            .map { row, _ -> row.toVideo() }
            .awaitSingleOrNull()

    override suspend fun findByUserId(userId: String): List<Video> =
